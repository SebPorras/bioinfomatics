...
try:
    ...
    if failed:
        ...
        sc.write_to_fasta(
            original_df[original_df.Entry.isin(failed_names)],
        ...
        merged_df = pd.merge(
            original_df, full_df, how="left", on=["Entry"], suffixes=["", "_r"]
        )
    ...
    print (f'{snakemake.input[0]} was empty')
    with open (f"{snakemake.input.workdir}/empty_brenda_entries.txt", "a+") as empty:
        empty.write(f'{snakemake.input[0]}\n')
