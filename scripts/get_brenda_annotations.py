...
def add_col_from_brenda_dict(df, entry_id, cols_to_add, brenda_dict):
    # print (brenda_dict)
    data = pd.DataFrame({'Entry' : [entry_id]}) 
    df = pd.concat([data, df])
#     print (df)
    for name, annots in brenda_dict.items():
        # data = pd.DataFrame({entry_id: ";".join(
...
print ('Called the BRENDA ANNOTATIONS SCRIPT')

ec_nums = [snakemake.wildcards.ec_num.replace("_", ".")]
...
if ec_nums:
    ...
    for ec_num in ec_nums:
        ...
        for entry_id, bd in brenda_dict.items():
            ...
            original_df = add_col_from_brenda_dict(original_df, entry_id, bd.keys(), bd)
