...
def auto_generate_filtered_entries(threshold: int, entry_limit, row_num):
    ...
    df = df.loc[df['Entry'].isin(target_ip)]

    families = df['Protein_families'].unique()
   
    chosen_seqs = set(df.loc[df['Protein_families'].isin(families)])

    final_seqs.update(chosen_seqs)
